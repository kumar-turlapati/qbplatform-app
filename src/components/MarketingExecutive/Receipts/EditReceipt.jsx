...
export const EditReceipt = ({navigation, route}) => {
      ...
      <View style={{backgroundColor: 'white', marginTop: 17}}>
        ...
        <View style={styles.viewStyle}>
          ...
          <TextInput
            ...
            onChangeText={amount => {
              setAmountPaid(parseFloat(amount));
              setIsFormUpdated(true);
            }}
          />
        </View>
  ...
  const updateReceipt = async () => {
    ...
    await axios
      .put(
        restEndPoints.UPDATE_RECEIPT.URL(receiptNo),
        {
          narration: narration,
      ...
      .then(apiResponse => {
        ...
        if (apiResponse.data.status === 'success') {
          ...
          setShowAlertMessage(`Receipt no. ${receiptNo} updated successfully.`);
        } else {
          setAlertTitle('Oops :(');
